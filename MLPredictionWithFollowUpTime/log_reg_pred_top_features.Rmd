---
title: "Logistic Regression Predictions on top features"
author: "GMJ"
date: "2022-12-10"
...
```
```{r}
df = subset(df, select = c(DEATH_EVENT, ejection_fraction, serum_creatinine, month))
df
```
...
for(i in 1:n) {
  ...
  model = glm(DEATH_EVENT ~  ejection_fraction  + serum_creatinine + as.factor(month), data = train_data, family = "binomial")
...
```{r}
write.csv(final_result, "log_reg_pred_top_features_results.csv")
```

```{r}
write.csv(results_df, "log_reg_pred_top_features_results_log.csv")
```